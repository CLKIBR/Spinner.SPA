...
export class MalCollectionService {
  ...
  // MalCollection verilerini sayfalama ile almak için
  getMalCollection(pageIndex: number = 0, pageSize: number = 10): Observable<any> {
    const url = `${this.path}/MalCollections?PageIndex=${pageIndex}&PageSize=${pageSize}`;
  ...
  // MalCollection silme işlemi
  deleteMalCollection(id: string): Observable<any> {
    const url = `${this.path}/MalCollections/${id}`;
  ...
  getMalCollectionById(id: string): Observable<any> {
    const url = `${this.path}/MalCollections/${id}`; // id'ye göre veri çekmek için URL
  ...
  updateMalCollection(id: string, updatedMalCollection: string): Observable<any> {
    return this.httpClient.put<any>(`${this.path}/MalCollections/`, updatedMalCollection,{headers : this.createHeaders()});
  }

  addMalCollection(newMalCollection: any): Observable<any> {
    const url = `${this.path}/MalCollections`; // API'nin POST endpoint'i
    return this.httpClient.post<any>(url, newMalCollection,{headers : this.createHeaders()}).pipe(
      catchError(error => {
