...
export class MalNumberService {
  ...
  // MalNumber verilerini sayfalama ile almak için
  getMalNumber(pageIndex: number = 0, pageSize: number = 10): Observable<any> {
    const url = `${this.path}/MalNumbers?PageIndex=${pageIndex}&PageSize=${pageSize}`;
  ...
  // MalNumber silme işlemi
  deleteMalNumber(id: string): Observable<any> {
    const url = `${this.path}/MalNumbers/${id}`;
  ...
  getMalNumberById(id: string): Observable<any> {
    const url = `${this.path}/MalNumbers/${id}`; // id'ye göre veri çekmek için URL
  ...
  updateMalNumber(id: string, updatedMalNumber: string): Observable<any> {
    return this.httpClient.put<any>(`${this.path}/MalNumbers/`, updatedMalNumber, { headers: this.createHeaders() });
  }

  addMalNumber(newMalNumber: any): Observable<any> {
    const url = `${this.path}/MalNumbers`; // API'nin POST endpoint'i
    return this.httpClient.post<any>(url, newMalNumber, { headers: this.createHeaders() })
      .pipe(
        catchError(error => {
