...
export class MalGroupService {
  ...
  getMalGroup(pageIndex: number = 0, pageSize: number = 10): Observable<any> {
    const url = `${this.path}/MalGroups?PageIndex=${pageIndex}&PageSize=${pageSize}`;
  ...
  deleteMalGroup(id: string): Observable<any> {
    const url = `${this.path}/MalGroups/${id}`;
  ...
  getMalGroupById(id: string): Observable<any> {
    const url = `${this.path}/MalGroups/${id}`; // id'ye göre veri çekmek için URL
  ...
  updateMalGroup(id: string, updatedMalGroup: string): Observable<any> {
    return this.httpClient.put<any>(`${this.path}/MalGroups/`, updatedMalGroup,{headers : this.createHeaders()});
  }

  addMalGroup(newMalGroup: any): Observable<any> {
    const url = `${this.path}/MalGroups`; // API'nin POST endpoint'i
    return this.httpClient.post<any>(url, newMalGroup,{headers : this.createHeaders()}).pipe(
      catchError(error => {
